...
for j,i in enumerate(girl):
	image_name = "magic_img/MagicGirl_"+str(j+1)+".png"
	data= i.split(",")
	name = data[0]
	age = data[1]
	desc = data[2]
	...
	ma = Mahou_Shoujo.objects.create(name=name, age=age, desc=desc, image=picture)
	a.append(ma)

string2='''吸塵器,可以透過魔杖吸取身邊人們的一項才能，但同時要交換一項等值的才能、記憶或是金錢。
...
for j,i in enumerate(wand):
	image_name = "magic_img/wand_"+str(j+1)+".png"
	data= i.split(",")
	name = data[0]
	desc = data[1]
	content = open(image_name, 'rb').read()
	picture = InMemoryUploadedFile(file=BytesIO(content), field_name="file", name="image_name", content_type="image/png", size=len(content), charset=None)
	Magic_Wand.objects.create(name=name, desc=desc, image=picture, mahou_shoujo=a[j])